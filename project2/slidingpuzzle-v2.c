...
// Prints the log of moves made during the game
void printMoves() {
...
/*
* prints the board by passing to formatted function
...
int isValid(int r, int c) { // "Is Move Valid?"
    ...
    if (((c - 1) <= zeroCol && zeroCol <= (c + 1)) &&
        ...
        if ((zeroCol == c && zeroRow == r) || (zeroRow == r + 1 && zeroCol == c + 1) || (zeroRow == r - 1 && zeroCol == c - 1) ||
...
*/
void moveCommand(int input) { // "Move Tile"
    int c, r, x, y;
    map(&x, &y, 0);
    map(&r, &c, input);
    if (isValid(r, c) == 1) {
        ...
        printf("Invalid Move. Please try a new command.\n");
    }
}
