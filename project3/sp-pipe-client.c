    ...
    while (temp != 'q') {
        ...
        if (gameOver == 0) {
            ...
            if (temp == 'm') {
                ...
                scanf("%d", &d);
                write(pipe_fd[1], &temp, sizeof(temp)); // writing move command and number to pipe
                ...
                char fileName[100];
                scanf("%s",fileName); 
                write(pipe_fd[1], &temp, sizeof(temp)); // write s to load
                write(pipe_fd[1], &fileName, sizeof(fileName)); // write filename
                read(pipe_fd[0], &temp, sizeof(temp));
                if (temp == 1) {
                    printf("File saved!\n");
                } else {
                ...
                char fileName[100];
                scanf("%s",fileName); 
                write(pipe_fd[1], &temp, sizeof(temp)); // write l to load
                write(pipe_fd[1], &fileName, sizeof(fileName)); // write filename
                read(pipe_fd[0], &temp, sizeof(temp)); // is read if file exists
                if (temp == 1) {
                    readAll();
                    printf("File loaded!\n");
                } else {
                ...
                printf("Game saved!\n");
            } else if (temp == 'n') { // NEWGAME
                free(boardy);
                printf("Enter a size to move.\n");
                scanf("%d", &row);
                col = row;
                write(pipe_fd[1], &row, sizeof(row));
                boardy = malloc(row * col * sizeof(int *));
                readAll();
            } else if (temp == 'q') {
                endClient();
